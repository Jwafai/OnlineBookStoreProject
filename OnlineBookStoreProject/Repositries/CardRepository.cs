...
namespace OnlineBookStoreProject.Repositries
{
    public class CardRepository : ICartRepository
    {
        ...
        public CardRepository(DatabaseContext db,IHttpContextAccessor httpContextAccessor ,UserManager<IdentityUser> userManager)
        {
            _db = db;
        ...
        public async Task<int> AddItem(int bookId, int qty)
        {
            ...
            if(cart is null)
            {
                ...
                _db.SaveChanges();
                var cartItem = _db.CartDetail.FirstOrDefault(a => a.ShoppingCartId == cart.Id && a.BookId==bookId);
        ...
        public async Task<int> RemoveItem(int bookId)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(userId))
                {
                    throw new Exception("user nor logged-in");
                }
                var cart = await GetCart(userId);
                if (cart is null)
                {
        ...
        {
            var cart = _db.ShoppingCart.FirstOrDefaultAsync(x => x.UserId == userId);
            return cart;
